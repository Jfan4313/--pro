...
import { CheckCircle2, KeyRound, Loader2, Plus, Search, ShieldCheck, UserRound, UsersRound, XCircle } from "lucide-react";
import { apiClient } from "@/src/lib/apiClient";
import { useAuth, type AuthUser } from "@/src/lib/auth";

const roles = [
  { id: "admin", label: "系统管理员", note: "全部权限，包括帐号和系统设置" },
...
export function AccountManagement() {
  ...
  const createAccount = async (event: FormEvent) => {
    ...
    if (form.password.length < 8) return window.dispatchEvent(new CustomEvent("show-toast", { detail: "临时密码至少需要 8 位" }));
    setSaving(true);
    try {
      ...
      setShowCreate(false);
      window.dispatchEvent(new CustomEvent("show-toast", { detail: "帐号已创建，首次登录需要修改密码" }));
    } catch (error: any) {
      window.dispatchEvent(new CustomEvent("show-toast", { detail: error?.status === 409 ? "该登录帐号已存在" : "帐号创建失败，请检查填写内容" }));
  ...
  const updateAccount = async (account: AuthUser, changes: Partial<AuthUser>) => {
    ...
    } catch { window.dispatchEvent(new CustomEvent("show-toast", { detail: "无法更新该帐号" })); }
  };
  ...
  return <div className="min-h-full bg-slate-50 p-4 md:p-8"><div className="mx-auto max-w-6xl space-y-5">
    <header className="flex flex-col gap-4 rounded-[28px] bg-slate-950 p-5 text-white md:flex-row md:items-center md:justify-between md:p-7"><div><p className="text-xs font-bold text-indigo-300">系统安全</p><h2 className="mt-1 text-2xl font-bold">帐号与权限</h2><p className="mt-2 text-sm text-slate-400">管理成员登录、角色、停用状态和密码</p></div><button onClick={() => setShowCreate(true)} className="flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 text-sm font-bold"><Plus className="h-4 w-4" />新增帐号</button></header>
  ...
  </div>{showCreate && <div className="fixed inset-0 z-[90] flex items-end justify-center bg-slate-950/50 md:items-center md:p-6"><form onSubmit={createAccount} className="mobile-sheet max-h-[92dvh] w-full max-w-lg overflow-y-auto rounded-t-[28px] bg-white p-5 shadow-2xl md:rounded-3xl md:p-6"><div className="flex items-center justify-between"><div><h3 className="text-lg font-bold text-slate-900">新增帐号</h3><p className="mt-1 text-xs text-slate-500">成员首次登录后需要修改临时密码</p></div><button type="button" onClick={() => setShowCreate(false)} className="rounded-full bg-slate-100 p-2 text-slate-500"><XCircle className="h-5 w-5" /></button></div><div className="mt-5 grid gap-4 sm:grid-cols-2">{[["姓名", "name", "例如：张伟"], ["登录帐号", "username", "英文、数字或点号"], ["邮箱", "email", "可选"], ["手机号", "phone", "可选"]].map(([label, key, placeholder]) => <label key={key} className="block"><span className="mb-1.5 block text-sm font-semibold text-slate-700">{label}</span><input value={(form as any)[key]} onChange={(event) => setForm({ ...form, [key]: event.target.value })} placeholder={placeholder} className="survey-input" /></label>)}<label className="block sm:col-span-2"><span className="mb-1.5 block text-sm font-semibold text-slate-700">角色</span><select value={form.role} onChange={(event) => setForm({ ...form, role: event.target.value })} className="survey-input">{roles.map((role) => <option key={role.id} value={role.id}>{role.label} — {role.note}</option>)}</select></label><label className="block sm:col-span-2"><span className="mb-1.5 block text-sm font-semibold text-slate-700">临时密码</span><input type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} placeholder="至少 8 位" className="survey-input" /></label></div><button disabled={saving} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 py-3 text-sm font-bold text-white disabled:opacity-50">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserRound className="h-4 w-4" />}{saving ? "正在创建" : "创建帐号"}</button></form></div>}</div>;
}

function Metric({ label, value, tone = "text-slate-900" }: { label: string; value: number; tone?: string }) { return <div className="rounded-2xl border border-slate-100 bg-white p-3 text-center shadow-sm md:p-4"><p className={`text-xl font-bold ${tone}`}>{value}</p><p className="mt-1 text-[10px] font-medium text-slate-500 md:text-xs">{label}</p></div>; }
