...
export function MobileHome({
  ...
  const metrics = [
    ...
    { label: "待审批", value: pendingApprovals, tone: "bg-slate-100 text-slate-700", tab: "dashboard" },
  ];
      ...
      <section className="mt-5 rounded-3xl border border-slate-100 bg-white p-4 shadow-sm">
        ...
        <div className="mt-3 space-y-2">
          {risks.slice(0, 3).map((risk: any) => (
            <button key={risk.id} onClick={() => setActiveTab(risk.actionTab)} className="flex w-full items-start gap-3 rounded-2xl bg-slate-50 p-3 text-left">
