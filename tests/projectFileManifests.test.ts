...
test("文件清单发布支持远程只读状态并标记来源电脑已缺失", () => {
  const db = makeDb();
  upsertManifests({ db, user, nowIso, items: [{ id: "f1", projectId: "p1", stageId: "1_initiation", originalName: "现场勘察.pdf", relativePath: "01/现场勘察.pdf", size: 12, sourceClientId: "client-a" }] });
  assert.equal(listManifests({ db, user, projectId: "p1" })[0].availability, "local-only");
  upsertManifests({ db, user, nowIso, items: [{ id: "f2", projectId: "p1", stageId: "1_initiation", originalName: "项目概况.pdf", relativePath: "01/项目概况.pdf", size: 18, sourceClientId: "client-a" }] });
...
test("上传分片完成后才把清单标记为已上传并校验哈希", () => {
  const db = makeDb();
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "zhijian-manifest-test-"));
  try {
    const content = Buffer.from("project-file-content");
    upsertManifests({ db, user, nowIso, items: [{ id: "f1", projectId: "p1", stageId: "1_initiation", originalName: "资料.txt", relativePath: "资料.txt", size: content.length, sourceClientId: "client-a" }] });
    ...
    completeUpload({ db, user, upload, checksum, nowIso });
